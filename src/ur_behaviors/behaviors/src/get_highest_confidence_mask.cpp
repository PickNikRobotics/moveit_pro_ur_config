...
{
  return {
    BT::InputPort<std::vector<moveit_studio_vision_msgs::msg::Mask2D>>(
        kPortIDMasks2D, "{masks2d}", "Vector of 2D masks from segmentation."),
    BT::InputPort<std::vector<double>>(
        kPortIDConfidenceScores, "{confidence_scores}", "Confidence scores parallel to masks2d."),
    BT::OutputPort<moveit_studio_vision_msgs::msg::Mask2D>(
        kPortIDBestMask, "{best_mask}", "The mask with the highest confidence."),
    BT::OutputPort<double>(
        kPortIDBestScore, "{best_score}", "The highest confidence score."),
  };
}
...
{
  return {
    { moveit_pro::behaviors::kDescriptionMetadataKey, kDescriptionGetHighestConfidenceMask },
...
BT::NodeStatus GetHighestConfidenceMask::tick()
{
  ...
  {
    shared_resources_->logger->publishFailureMessage(name(),
        "Mask count (" + std::to_string(masks.size()) + ") does not match score count (" +
  ...
  shared_resources_->logger->publishInfoMessage(name(),
      "Selected mask " + std::to_string(best_index) + " with confidence " +
      std::to_string(*best_it) + " out of " + std::to_string(masks.size()) + " masks.");
